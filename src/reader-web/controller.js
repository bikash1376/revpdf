...
(function () {
  ...
  // Grow a caret hit into the word under the finger — a long-press that selects
  // a single character would be useless.
  function wordRangeAt(doc, x, y) {
    var r = caretRangeAt(doc, x, y);
  ...
  // Build a non-collapsed Range from two boundary points in unknown order.
  // setEnd() before start silently collapses rather than throwing, so we detect
  // that and retry the other way round.
  function orderedRange(doc, p1, p2) {
    var r = doc.createRange();
    try {
      r.setStart(p1.c, p1.o);
      r.setEnd(p2.c, p2.o);
      if (!r.collapsed) return r;
    } catch (e) {}
    var r2 = doc.createRange();
    try {
      r2.setStart(p2.c, p2.o);
      r2.setEnd(p1.c, p1.o);
  ...
  function selClearPaint(host) {
    ...
    while (host.layer.firstChild) host.layer.removeChild(host.layer.firstChild);
  }
  ...
  function selPaint() {
    ...
    // Handles, per the reference: a dot above the caret bar at the start of the
    // selection, and a dot below the bar at the end.
    var first = rects[0];
    var last = rects[rects.length - 1];

    var start = doc.createElement('div');
    start.className = 'rp-sel-h';
    start.setAttribute('data-h', 'start');
    start.style.left = first.left - 14 + 'px';
    start.style.top = first.top - 14 + 'px';
    start.style.height = first.height + 14 + 'px';
    start.innerHTML =
      '<i style="top:0"></i><b style="top:14px;height:' + first.height + 'px"></b>';
    frag.appendChild(start);

    var end = doc.createElement('div');
    end.className = 'rp-sel-h';
    end.setAttribute('data-h', 'end');
    end.style.left = last.right - 14 + 'px';
    end.style.top = last.top + 'px';
    end.style.height = last.height + 14 + 'px';
    end.innerHTML =
      '<b style="top:0;height:' + last.height + 'px"></b><i style="top:' + last.height + 'px"></i>';
    frag.appendChild(end);

    host.layer.appendChild(frag);
  }
  ...
  function selRepaint() {
    ...
    selRaf = requestAnimationFrame(function () {
      selRaf = 0;
      selPaint();
      if (selCur) selPost(); // the rect moved, so RN's buttons must move too
    });
  }
  ...
  function selMoveDrag(host, touch) {
    var doc = host.doc;
    var x = touch.clientX + selDragOff.x;
    var y = touch.clientY + selDragOff.y;
    var hit = caretRangeAt(doc, x, y);
    if (!hit || !hit.startContainer) return;
    var rng = selCur.range;
    var fixed =
      selDrag === 'start'
        ? { c: rng.endContainer, o: rng.endOffset }
        : { c: rng.startContainer, o: rng.startOffset };
    var moving = { c: hit.startContainer, o: hit.startOffset };
    var next = orderedRange(doc, fixed, moving);
    if (!next) return;
    // Dragging one handle past the other flips which end it now controls.
    selDrag =
      next.startContainer === moving.c && next.startOffset === moving.o ? 'start' : 'end';
    selCur.range = next;
    selPaint();
  }
  ...
  function selInstall(doc, kind, frame, contents) {
    ...
    var host = { doc: doc, kind: kind, frame: frame || null, contents: contents || null, layer: null };
    ...
    // If the user asked for the OS menu, leave the platform's selection alone
    // and don't run ours — the two can't coexist.
    if (!allowNativeMenu) body.classList.add('rp-nosel');
    ...
    doc.addEventListener(
      'touchstart',
      function (e) {
        if (allowNativeMenu) return;
        var t = e.touches[0];
        if (!t) return;

        var handle =
          e.target && e.target.closest ? e.target.closest('.rp-sel-h') : null;
        if (handle && selCur) {
        ...
        // A tap anywhere else dismisses the current selection rather than
        // toggling the reader chrome.
        if (selCur) {
          selClear(false);
          suppressTapUntil = Date.now() + 400;
          return;
        ...
        lpTimer = setTimeout(function () {
          lpTimer = null;
          if (moved) return;
          var r = wordRangeAt(doc, sx, sy);
          if (!r) return;
          selCur = { host: host, range: r };
          selDrag = null;
          suppressTapUntil = Date.now() + 600;
          selPaint();
          selPost();
    ...
    function endTouch() {
      ...
      if (selDrag) {
        selDrag = null;
        selPost(); // commit the dragged range (and its new rect) to RN
      }
    }
  ...
  window.RP = {
    loadBook: function (b64, format, location) {
      try {
        ...
        // Only epub.js' own selection path when the user explicitly asked for the
        // OS menu; otherwise our engine owns selection (see selInstall below).
        rendition.on('selected', function (cfiRange, contents) {
          ...
          if (text && text.trim().length) {
            suppressTapUntil = Date.now() + 400;
            post({ type: 'selection', text: text.trim(), cfiRange: cfiRange, anchor: cfiRange });
          }
        });
    ...
    setNativeMenu: function (enabled) {
      ...
      // Opting into the OS menu means giving the platform its selection back:
      // drop our user-select:none clamp and any overlay we're currently drawing.
      selClear(true);
      selHosts.forEach(function (h) {
        var body = h.doc.body || h.doc.documentElement;
        if (!body) return;
        if (allowNativeMenu) body.classList.remove('rp-nosel');
        else body.classList.add('rp-nosel');
      });
    },
